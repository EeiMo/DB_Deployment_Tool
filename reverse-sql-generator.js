...
class ReverseSQLGenerator {
    constructor(schemaData = null) {
        ...
        this.patterns = {
            ...
            ALTER_ADD_COLUMN: /ALTER\s+TABLE\s+([`"]?[\w\-\.]+[`"]?)\s+ADD\s+(COLUMN\s+)?([`"]?[\w\-]+[`"]?)(?:\s+([^,;]+))?/i,
        ...
        for (const line of lines) {
            ...
            if (commentOnColumnMatch && (!isCreateTableOnlyFile || isCommentOnlyFile)) {
                ...
                if (tableInfo && tableInfo.columns && tableInfo.columns[columnName]) {
                    ...
                    reverseScript += `-- 警告: 无法找到表 ${tableName} 或列 ${columnName} 的原始注释信息\n`;
                    reverseScript += `-- 请手动恢复: COMMENT ON COLUMN ${tableName}.${columnName} IS '[原始注释]';\n\n`;
                }
                continue;
            }
            
            // 其他操作的处理可以在这里添加
            reverseScript += `-- 未处理的DDL操作: ${line}\n`;
        }
    ...
    reverseCommentOnColumn(statement) {
        ...
        // 如果没有找到原始注释，生成注释说明
        let reverseSQL = `-- 原语句: ${statement.trim()}\n`;
        reverseSQL += `-- 未找到列 ${tableName}.${columnName} 的相关字段与注释`;
        return reverseSQL;
    }
