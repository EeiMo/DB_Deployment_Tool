...
const LEFT_WIDTH_KEY = 'autoCreateTable:leftWidth';
...
// 调整工作区左右宽度的分隔条
function attachWorkspaceSplitter() {
  ...
  if (!Number.isNaN(saved) && saved > 0) {
    rootStyle.setProperty('--left-pane-width', `${saved}px`);
  }
  ...
  const onMove = (e) => {
    ...
    rootStyle.setProperty('--left-pane-width', `${next}px`);
  };
  ...
  const onUp = () => {
    ...
    const curr = leftPane ? leftPane.clientWidth : parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--left-pane-width')) || 460;
    if (!Number.isNaN(curr)) {
      localStorage.setItem(LEFT_WIDTH_KEY, String(curr));
    }
  };
  ...
  splitter.addEventListener('mousedown', (e) => {
    ...
    startX = e.clientX;
    const leftPane = document.querySelector('.left-pane');
    startW = leftPane ? leftPane.clientWidth : parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--left-pane-width')) || 460;
...
function buildSqlPreview() {
  ...
  if (schema) html += `${kw('SET')} ${kw('SEARCH_PATH')} = <span class="hl-schema">${escapeHtml(schema)}</span>;\n`;
  if (schema && table) {
    sql += `CREATE TABLE ${schema}.${table} (\n`;
    sql += columnDefs + '\n';
    sql += `) WITH(ORIENTATION = COLUMN, COMPRESSION = LOW, COLVERSION = 2.0, ENABLE_DELTA = FALSE)\n`;
    html += `${kw('CREATE')} ${kw('TABLE')} <span class="hl-schema">${escapeHtml(schema)}</span>.<span class="hl-table">${escapeHtml(table)}</span> (\n`;
    html += Array.from(colMap.entries())
      .map(([name, v]) => `  <span class="hl-column">${escapeHtml(name)}</span> <span class="hl-type">${escapeHtml(v.type)}</span>`)
      .join(',\n') + '\n';
    html += `) ${kw('WITH')}(${kw('ORIENTATION')} = ${kw('COLUMN')}, ${kw('COMPRESSION')} = ${kw('LOW')}, ${kw('COLVERSION')} = 2.0, ${kw('ENABLE_DELTA')} = ${kw('FALSE')})\n`;
    const keys = distributeInput.split(',').map(s => String(s).trim()).filter(Boolean);
    if (keys.length > 0) {
      sql += `DISTRIBUTE BY HASH(${keys.join(', ')})\n`;
      html += `${kw('DISTRIBUTE')} ${kw('BY')} ${kw('HASH')}(${keys.map(k => `<span class=\"hl-distribute-key\">${escapeHtml(k)}</span>`).join(', ')})\n`;
    } else {
      sql += `DISTRIBUTE BY ROUNDROBIN\n`;
      html += `${kw('DISTRIBUTE')} ${kw('BY')} ${kw('ROUNDROBIN')}\n`;
    }
    ...
    html += `${kw('TO')} ${kw('GROUP')} \"LC_DW1\"${hasPartition ? '\n' : ';\n'}`;
    ...
    if (hasPartition) {
      ...
      linesHtml.push(`  ${kw('PARTITION')} P190001 ${kw('VALUES')} ${kw('LESS')} ${kw('THAN')}(${fmtPeriod(pMin.year, pMin.month)}) ${kw('TABLESPACE')} ${kw('PG_DEFAULT')},`);
      ...
      while (lessThan(cur, pMax)) {
        ...
        linesHtml.push(`  ${kw('PARTITION')} P${curStr} ${kw('VALUES')} ${kw('LESS')} ${kw('THAN')}(${nxtStr}) ${kw('TABLESPACE')} ${kw('PG_DEFAULT')},`);
        cur = nxt;
      }
      // 固定最大分区名 P471212，对应 MAXVALUE
      lines.push(`  PARTITION P471212 VALUES LESS THAN(MAXVALUE) TABLESPACE PG_DEFAULT`);
      linesHtml.push(`  ${kw('PARTITION')} P471212 ${kw('VALUES')} ${kw('LESS')} ${kw('THAN')}(MAXVALUE) ${kw('TABLESPACE')} ${kw('PG_DEFAULT')}`);
      sql += `PARTITION BY RANGE(PERIOD_ID)\n(\n${lines.join('\n')}\n);\n`;
      html += `${kw('PARTITION')} ${kw('BY')} ${kw('RANGE')}(<span class=\"hl-column\">PERIOD_ID</span>)\n(\n${linesHtml.join('\n')}\n);\n`;
    }
  }

  if (schema && table && tableComment) {
    sql += `COMMENT ON TABLE ${schema}.${table} IS '${escapeSqlLiteral(tableComment)}' ;\n`;
    html += `${kw('COMMENT')} ${kw('ON')} ${kw('TABLE')} <span class="hl-schema">${escapeHtml(schema)}</span>.<span class="hl-table">${escapeHtml(table)}</span> ${kw('IS')} '<span class="hl-comment">${escapeHtml(tableComment)}</span>' ;\n`;
  }
  ...
  for (const [name, v] of colMap.entries()) {
    if (v.comment) {
      ...
      html += `${kw('COMMENT')} ${kw('ON')} ${kw('COLUMN')} <span class="hl-schema">${escapeHtml(schema)}</span>.<span class="hl-table">${escapeHtml(table)}</span>.<span class="hl-column">${escapeHtml(name)}</span> ${kw('IS')} '<span class="hl-comment">${escapeHtml(v.comment)}</span>' ;\n`;
    }
  }

  $('sqlPreview').innerHTML = (html || escapeHtml('请先选择映射文件并配置列映射'));
